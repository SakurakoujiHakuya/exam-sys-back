...
import com.exam.system.domain.MessageUser;
import com.exam.system.domain.User;
import com.exam.system.domain.UserEventLog;
import com.exam.system.domain.enums.RoleEnum;
...
import com.exam.system.service.MessageService;
import com.exam.system.service.UserEventLogService;
...
import com.exam.system.viewmodel.student.user.*;
...
public class UserController extends BaseWXApiController {
    ...
    public RestResponse register(@Valid UserRegisterVM model) {
        ...
        if (null != existUser) {
            return new RestResponse<>(2, "用户已存�?);
        }
    ...
    public RestResponse<UserResponseVM> update(@Valid UserUpdateVM model) {
        ...
        UserEventLog userEventLog = new UserEventLog(user.getId(), user.getUserName(), user.getRealName(), new Date());
        userEventLog.setContent(user.getUserName() + " 更新了个人资�?);
        eventPublisher.publishEvent(new UserEvent(userEventLog));
        UserResponseVM userVm = UserResponseVM.from(user);
        return RestResponse.ok(userVm);
    }
